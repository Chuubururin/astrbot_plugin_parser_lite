...
- **配置**：WebUI 插件页共 29 个配置项（3 桥接 + 26 上游），保存后自动重载
- **版本与许可证**：随上游快照滚动自动再生（当前上游 v1.3.7-pre-release.4），与 release tag 锁步；许可证随上游（MIT），vendor 快照零修改
...
- **出站 SSRF 防护**：媒体下载出站经四层校验（scheme 白名单 → 端口规则 →
  解析地址逐 IP 校验 → 「解析即连接」钉扎防 DNS rebinding）。唯一例外是
  ffmpeg HLS 子进程出站：仅入口 URL 校验 + 启动后自毁监听，存在子进程无法
  钉扎 IP 的 DNS TOCTOU 残窗（`bridge/vendor_patches.py` 头注如实声明）。
