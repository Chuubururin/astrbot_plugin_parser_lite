...
**URL 候选裁定**
消息里可能同时出现多条 URL，候选按固定次序扫描（纯文本先于 JSON 卡片，
源内按出现顺序），首个被 vendor `match()` 接受者胜出。桥不建打分 / 排序 /
多候选并行重试层——「哪条 URL 属于哪个平台」的裁判权完整留在 vendor
（唯一真值源），桥侧任何优先级启发式都会在每次 roll 时变成待对表的第二
套裁定语义。
