...
| # | 门禁 | 命令 | 说明 |
|---|---|---|---|
| 1 | 测试 | `python -m pytest -c config/pyproject.toml --rootdir=. -q` | 基线 **566 passed / 3 skipped**（本地与 CI 一致：CI 已浅克隆上游做活体对照；3 条 skip 均为 network 隔离区，设 `RUN_NETWORK_TESTS=1` 启用）。无上游克隆的裸环境为 **562 passed / 7 skipped**——多出的 4 条需要 `.sync-work/` 上游克隆（跳过原因写作「无上游克隆」），属设计内 |
